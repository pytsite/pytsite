...
def app_setup():
    ...
    for role in admin_role, user_role:
        if not manager.get_role(name=role[0]):
            role_entity = manager.create_role(role[0], role[1])
    ...
    admin_user = manager.create_user(email)
    admin_user.f_set('fullName', t('pytsite.auth@administrator'))
    admin_user.f_add('roles', manager.get_role('admin'))
    admin_user.save()
