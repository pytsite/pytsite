...
def app_setup():
    ...
    for role in admin_role, user_role:
        if not _functions.get_role(name=role[0]):
            ...
            role_entity.save()
            _console.print_success(_lang.t('auth@role_has_been_created', {'name': role_entity.f_get('name')}))
    ...
    try:
        ...
        admin_user.save()
        _console.print_success(_lang.t('auth@user_has_been_created', {'login': admin_user.f_get('login')}))
